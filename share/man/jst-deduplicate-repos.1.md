% JST-DEDUPLICATE-REPOS(1) | General Commands Manual

NAME
====

jst-deduplicate-repos - remove duplicate repositories from a
multi-repository configuration

SYNOPSIS
========

**`jst-deduplicate-repos`**  \[*`repository name`*\]...  
...
called `etc/repos.template.json` in **`jst-repo-config`**(5)
...
invocations of **`jst-import-git`**(1) which also adds the indirect
dependencies.
...
been fetched (possibly by a different project) to **`jst`**(1)'s
...
the work done by **`jst-import-git`**(1) manageable it requires the
...
chaining **`jst-import-git`**(1) this dependency will end up twice
...
redundant copies. To avoid this, **`jst-deduplicate-repos`** can
