...
Read the value of a runtime variable (the target is configured with).
...
> Note: The advantage of using this function over the [variable-read syntax](./getting-started.md#runtime-variables)
> `$'name'` is that a default value can be specified.
...
jst.env('a')    // identical to the variable-read syntax: $'a'
...
### `jst.zip_with(func, range1, range2)`

Run function for combining elements from two ranges.

| Parameter | Type | Description | Default value |
|-|:-|:-|:-:|
| `func`   | *Func* | The function to call       | None |
| `range1` | *List* | The first iteration range  | None |
| `range2` | *List* | The second iteration range | None |

> Note: `func` must be a function accepting exactly two arguments, the
> corresponding element from the first and second list. If the input ranges are
> of different length, any element with no correspondence in the other range is
> ignored.
...
**Example:**
``` jsonnet
local concat(a, b) = a + '_' + b;

// returns ['foo_fox', 'bar_bat']
jst.zip_with(concat, ['foo', 'bar'], ['fox', 'bat'])
```

---

### `jst.zip_map(range_key, range_val)`

Create map from elements of two ranges.

| Parameter | Type | Description | Default value |
|-|:-|:-|:-:|
| `range_key` | *List* | The key iteration range   | None |
| `range_val` | *List* | The value iteration range | None |

> Note: If the key and value ranges are of different length, any element with no
> correspondence in the other range is ignored.

**Returns:** The resulting elements as type *Map*.

**Example:**
``` jsonnet
// returns {'foo': 'fox', 'bar': 'bat'}
jst.zip_map(['foo', 'bar'], ['fox', 'bat'])

// identical to this code, but more efficient
local make_map(k, v) = {[k]: v};
jst.union(jst.zip_with(make_map, ['foo', 'bar'], ['fox', 'bat']))
```
...
> Note: `func` must be a function accepting exactly two arguments, the iteration
> variable first and the accumulation variable second.
