...
its run time. These actions can be associated with targets via the
`"origins"` field in the action graph that can be obtained via the
`--dump-graph` option.
...
invocations. To do so, `jst` honors an option `"invocation log"`
...
So, if we have `~/.jstrc` with contents
``` json
{
  "rc files": [
    {
...
{
  "invocation log": {
    "project id": "hot-new-product"
  }
}
```
then for each build triggered through `jst` a new subdirectory
of `~/log/jst-build/hot-new-product` will be created. The name
...
over fixed periods of time (monthly, daily, hourly, etc.).

Of course, just creating an empty directory for each invocation
of `jst` is not very useful. Therefore, more subkeys can
be specified.
 ...
 - `"metadata"` specifies the file name, inside the invocation
...
`~/.jstrc` file could look as follows.
``` json
{
  "rc files": [
    {
...
available, it is usually a good idea to choose a `"system"` path
rather than a `"home"` one for `"directory"`. In any case, it is
...
`jst-build` source tree, under `doc/invocations-http-server` there
