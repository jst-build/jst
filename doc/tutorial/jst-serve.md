Dependency management: `jst serve`
==================================

`jst serve` starts a remote target-level cache service (for ease called from
...
- projects can have many dependencies. These tend to not be part of the build
  environment and are instead built from source, but with *jst-build* these
...
The way this service interacts with the other *jst-build* components is
...
`jst serve` to provide first- and third-party dependencies to a *jst-build*
project by expanding on a previously covered example scenario.

Basic usage of `jst serve`
--------------------------

To simplify usage, `jst serve` supports only one argument, the path to a
...
path `.jst-servec` in the current working directory.

Let us see what calling `jst serve` with an empty configuration file does and
interpret the result. Thus, using a configuration file with content

``` {.json srcname=".jst-servec"}
{}
```
...
running `jst execute` starts a single-node remote build execution service in
the current environment. Our `jst serve` dependency management service always
...
the remote-execution endpoint. Therefore, calling `jst serve` without
...
specified by the client and the one set up for the `jst serve` instance will
result in an error.
...
control on the service endpoint is desired. The `jst serve` configuration file
...
``` sh
$ jst [...] -R 127.0.0.1:9999 -r 127.0.0.1:8989
# or
$ jst -R 127.0.0.1:9999 -r 127.0.0.1:8989 [...]
```
...
As with `jst execute`, mTLS must be enabled when the service starts, and it
...
``` sh
$ jst [...] -R <serve_endpoint> [-r <execution_endpoint>] --tls-ca-cert <path_to_CA_cert> \
...
repository). For this purpose, `jst serve` can be configured to be made aware
...
``` {.jsonc srcname=".jst-servec"}
{
  ...
  "repositories": [
    {
      "root": "system",
      "path": "var/repos/third-party-distfiles"
    },
    {
      "root": "system",
      "path": "var/repos/project-foo"
    },
    {
      "root": "system",
      "path": "var/repos/project-bar"
    }
  ]
  ...
}
...
``` {.jsonc srcname=".jst-servec"}
{
  ...
  "local build root": {
    "root": "system",
    "path": "var/cache/serve-build-root"
  }
  ...
}
```
...
Importantly, the local build root, as is generally the case for *jst-build*, is
...
What we would like to do is use a `jst serve` instance to provide the `"fmt"`
...
- the target description is provided by repository `"fmt-targets-layer"`, made
  ...
  checkout `./tutorial-defaults`. What makes `jst serve` more than a simple
...
In the following we will showcase how a `jst serve` instance can be configured
...
Let us start with a basic `.jst-servec` configuration file, on which we can
...
Secondly, we can instruct *jst-build* to treat this repository as _absent_,
which means that `jst` will set up this repository root such that it is not
...
adding its path to the known repository list in the `.jst-servec` configuration
...
thus to be updated to include this checkout location, so `.jst-servec` now
reads
...
and the `.jst-servec` configuration file is
...
would have to move to the new server deployment location just the `.jst-servec`
...
be managed and served by a suitably set up `jst serve` instance. With different
...
Thankfully, `jst` accepts as an argument a _repository configuration_ file
...
Then we need to set up the _rc-file_ of `jst`. By default, `jst` always
tries to read such a configuration file, with the default location being
`$HOME/.jstrc`. Here we will pass this file explicitly, so we create file
`rc-file` in the current directory with content

``` {.json srcname="rc-file"}
{
  "local build root": {
    "root": "home",
    "path": "local-build-root"
  },
  "remote serve": {
...
the current directory ensures the file will be picked up by `jst`.
...
Lastly, it should be mentioned that the `jst` rc-file can be instructed to
