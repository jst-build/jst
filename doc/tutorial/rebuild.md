...
[reproducible](https://reproducible-builds.org/). *jst-build*
...
always possible to break reproducibility with bad actions, coming both
from rules that are not carefully written and from ad-hoc actions added
...
...
  'version.h': {
    type: 'generic',
    cmds: ["echo '#define VERSION \"0.0.0.'`date +%Y%m%d%H%M%S`'\"' > version.h"],
    outs: ['version.h'],
  },
...
```
...
To search for the root cause of non-reproducibility, `jst` has a
...
action cache, if present with the same inputs. So you would typically
...
INFO: Requested target '""//:""' with config: {}
...
understand what's going on. However, checking for reproducibility is
usually part of a larger quality-assurance process. To support the
automation of such processes, the findings can also be reported in
machine-readable form.

``` sh
$ jst rebuild --dump-flaky flakes.json --dump-graph actions.json
[...]
$ cat flakes.json
{
  "cache misses": [
...
generated output. The files themselves can be obtained via
`jst install-cas` as usual, allowing deeper comparison of the
outputs. The full definitions of the actions can be found in the action
graph, in the example dumped as well as `actions.json`; this definition also includes
...
Simply rebuilding on the same machine is a good way to detect embedded
...
A simple but effective way of modifying the build environment is the
option `-L`, which sets the local launcher: a list of strings that the
argument vector is prefixed with before the action is executed. The default
...
either for two different remote-execution endpoints, or comparing one
remote-execution endpoint to the local build. The latter is also a good
...
for `build`, with the `-r` option; the cache endpoint to compare against
