...
To run tests with *jst-build*, we do **not** have a dedicated `test`
subcommand. Instead, we consider tests to be a specific action that
...
(but still, we want only successful actions to be taken from cache). Rules
...
For the remainder of this section, we expect to have available the project files
resulting from successfully completing the tutorial section on
...
functionality of the `greet` library. To do so, we need to provide
a C++ source file that performs the actual testing and returns
non-`0` on failure. For simplicity, we do not use a testing
...
only fetched when testing is requested. Let us place this code in the
subdirectory `tests`
...
default test runner can be overridden by a different test runner from
...
``` {.jsonnet srcname="tutorial-defaults/CC/test/TARGETS"}
{}
```

as well as the file `tutorial-defaults/shell/test/TARGETS` with content

``` {.jsonnet srcname="tutorial-defaults/shell/test/TARGETS"}
{}
```

and the file `tutorial-defaults/shell` with content

``` {.jsonnet srcname="tutorial-defaults/shell/TARGETS"}
{
  defaults: {type: 'defaults'},
}
```

indicating that we simply use the defaults for the shell.
...
The result of the test target consists of 5 artifacts: `result` (containing
...
above artifacts. The test ran successfully; otherwise the reported
artifacts would have been tagged as `FAILED` in the output, and
*jst-build* would have returned the exit code `2`.
...
``` sh
$ jst build -P stdout tests greet
INFO: Found 3 repositories involved
INFO: Requested target 'tutorial//tests:greet' with config: {}
INFO: Target tainted ["test"].
INFO: Discovered 5 actions, 0 tree overlays, 3 trees, 1 blobs
...
To also strip *jst-build*'s `INFO:` prints from this output, the argument
`--log-limit 1` can be passed to the `jst` call.

Our test binary does not have any useful options for interacting with
it directly. When working with test frameworks, it can sometimes be
desirable to get hold of the test binary itself for manual
...
``` sh
$ jst analyse --request-action-input -1 tests greet
INFO: Found 3 repositories involved
INFO: Requested target 'tutorial//tests:greet' with config: {}
...
``` sh
$ jst install -o work --request-action-input -1 tests greet
INFO: Found 3 repositories involved
INFO: Requested target 'tutorial//tests:greet' with config: {}
INFO: Request is input of action #-1
INFO: Target tainted ["test"].
...
of a target; consider, e.g., the auxiliary target `DEV`
...
The test target for this shell test uses the rule
...
The result is similar, again containing the 5 artifacts and a single
...
the build of multiple test reports. The simplest way to do so
...
are tainted by `"test"`, the compound test target must be tainted by
the same string as well. To create the compound test target combining
...
as artifacts. Both tests ran successfully, as none of the artifacts in
the output above are tagged as `FAILED`.
...
also propagates the provided information appropriately; this means it
can also be used, e.g., as a target for [linting](lint.md), allowing all
code that went into all tests to be linted from a single target. The usage
...
As the output structure is again one tree per test, test suites can
