Target versus `jst.file()`, `jst.glob()`, and `jst.tree()`
===========================================================
...
already saw the `jst.tree()` reference. In this section, we will highlight in
...
As an example for this section we use
[GNU units](https://www.gnu.org/software/units/), where we want to patch
the standard units definition to add two units of area popular in German news.
...
take the top-level directory as the layer for this. As we also want to patch
the definition file, we add the subdirectory `files` as a logical
...
Patching a file: targets versus `jst.file()`
--------------------------------------------

Let's start by patching the source file `definitions.units`. While,
conceptually, we want to patch a third-party source file, we do *not*
modify the sources. The workspace root is a git tree and stays that way.
...
``` sh
$ echo {} > TARGETS.units
$ jst install -o . definitions.units
INFO: Found 4 repositories involved
INFO: Requested target 'units//:definitions.units' with config: {}
...
Our rules conveniently contain a rule `@'rules//patch:file'` to patch a
...
same name: `jst.file('definitions.units')`. The path is always taken
relative to the current module, as explicit file references are only
allowed for the current module. So we add this target and obtain as
`TARGETS.units` the following.

``` {.jsonnet srcname="TARGETS.units"}
{
  'definitions.units': {
    type: @'rules//patch:file',
    src: [jst.file('definitions.units')],
...
their inputs) and, in fact, to have a fixed command line.

``` sh
$ jst analyse definitions.units --dump-actions -
INFO: Found 4 repositories involved
INFO: Requested target 'units//:definitions.units' with config: {}
...
Building `"definitions.units"`, we find that the patch applied correctly

``` sh
$ jst build definitions.units -P definitions.units | grep -A 5 'German units'
INFO: Found 4 repositories involved
INFO: Requested target 'units//:definitions.units' with config: {}
...
Globbing source files: `jst.glob()`
-----------------------------------
...
...
  'data-draft': {
    type: 'install',
    deps: ['definitions.units', 'currency.units'],
  },
...
```
...
``` sh
$ jst analyse data-draft
INFO: Found 4 repositories involved
INFO: Requested target 'units//:data-draft' with config: {}
...
The disadvantage, however, is that we might miss newly added `.units`
files if we update and upstream has added new files. So we want all source files
that have the respective ending. The corresponding source reference is
`jst.glob()`. A glob expands to the *collection* of all *sources* that are
...
and the rationale behind it.
 ...
 - First of all, the artifact (and runfiles) map has an entry for each
   ...
   the source files. This is different from `jst.tree()`, where the artifact
   ...
   there are fewer entries to carry around during analysis and
   action-key computation, and the whole directory is "reserved" for
   that tree, avoiding staging conflicts when entries are added there
   later.
 - As a source reference, a glob expands to explicit source files;
   ...
   account. In our example, `jst.glob('*.units')` therefore
 ...
 - Matches are only considered at the top-level directory. In this way,
   ...
   globs would require traversing subdirectories, at a much higher
   cost. While the explicit `jst.tree()` reference allows recursive
   ...
   tree identifier without traversing the tree. Such a quick lookup
   would not be possible if matches had to be selected.

So, `jst.glob('*.units')` expands to all the relevant source
...
file due to a conflicting name. In our case, however, dropping the
source file in favour of the patched one is deliberate. For this, there
is the rule `@'rules//data:overlay'` taking the union of the artifacts of
the specified targets, accepting conflicts and resolving them in a
latest-wins fashion. Keep in mind that our target fields are lists, not
...
...
  data: {
    ...
    deps: [jst.glob('*.units'), 'definitions.units'],
  },
...
```
...
...
  'units-draft': {
    ...
    'pure C': ['YES'],
    srcs: [jst.glob('*.c')],
    'private-hdrs': [jst.glob('*.h')],
  },
...
```

The result basically works and shows that we have 5 source files in
total, giving 5 compile actions and one link action.

``` sh
$ jst build units-draft
INFO: Found 4 repositories involved
INFO: Requested target 'units//:units-draft' with config: {}
...
``` sh
$ jst install -o . strfunc.c
INFO: Found 4 repositories involved
INFO: Requested target 'units//:strfunc.c' with config: {}
INFO: Discovered 0 actions, 0 tree overlays, 0 trees, 0 blobs
INFO: Processed 0 actions, 0 cache hits.
INFO: Artifacts can be found in:
...
...
  ...
  'patched srcs': {
    type: @'rules//data:overlay',
    deps: [jst.glob('*.c'), 'strfunc.c'],
  },

  'strfunc.c': {
    type: @'rules//patch:file',
    src: [jst.file('strfunc.c')],
...
$ jst build units
INFO: Found 4 repositories involved
INFO: Requested target 'units//:units' with config: {}
...
To finish the example, we also add a default target (relying on the fact
that, if no target is specified, `jst` builds the lexicographically
first target), staging artifacts according to the usual conventions.

``` {.jsonnet srcname="TARGETS.units"}
...
  '': {
    type: 'install',
    dirs: [['units', 'bin'], ['data', 'share/units']],
  },
...
```
...
``` sh
$ jst install -o /tmp/testinstall
INFO: Found 4 repositories involved
INFO: Requested target 'units//:""' with config: {}
