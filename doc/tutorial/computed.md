...
an exported top-level target staging all those files. This can
...
``` {.jsonnet srcname="utils/targets.generate"}
{
  '': {
    type: 'export',
    target: 'generate',
...
From that, we can, step by step, define the actual build description.
 ...
 - The tasks to generate the target files form a computed root of
   `"src target tasks description"`, using the top-level target
   `//:""`. We call it `"src target tasks"`.
...
INFO: Requested target '""//bar:""' with config: {}
...
INFO: Requested target '""//baz:""' with config: {}
...
Obviously, the tree structure has changed, so the `"src target tasks
description"` target gets rebuilt. The `"src target build"` target
gets rebuilt as well, but if we inspect the log, we see that 2 out
of 3 actions are taken from cache.

A similar construction is also used in the `jst-build` main `git`
