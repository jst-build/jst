...
*jst-build* needs to know the root of the project worked on. By default,
it searches upwards from the current directory until it finds a marker.
Currently, we support three different markers: the files `ROOT` and
`WORKSPACE` or the directory `.git`. Let's create a new project by
...
To build a target, we need to run `jst` with the subcommand `build`:

``` sh
$ jst build greeter
INFO: Requested target '""//:greeter' with config: {}
...
description of the artifact consisting of a `git` blob identifier, size,
...
without executing any actions, as all actions are served from
...
to standard error, this can be used both for interactively reading
a text file and for piping the artifact to another program.

``` sh
$ jst build greeter -P out.txt
INFO: Requested target '""//:greeter' with config: {}
INFO: Discovered 1 actions, 0 tree overlays, 0 trees, 0 blobs
INFO: Processed 1 actions, 1 cache hits.
INFO: Artifacts built, logical paths are:
...
In the last example we relied on the fact that, if no target is
specified, the lexicographically first one is taken.

Alternatively, we could also directly request the artifact `out.txt`
from *jst-build*'s CAS (content-addressable storage) and print it on
the command line via:

``` sh
$ jst install-cas [557db03de997c86a4a028e1ebd3a1ceb225be238:12:f]
Hello World
$
```
...
``` sh
$ jst install-cas 557db03de997c86a4a028e1ebd3a1ceb225be238
Hello World
$
```
...
allow requests specifying an output file, for *jst-build* this would
...
``` sh
$ jst build upper -p
INFO: Requested target '""//:upper' with config: {}
INFO: Discovered 1 actions, 0 tree overlays, 0 trees, 0 blobs
INFO: Processed 1 actions, 0 cache hits.
INFO: Artifacts built, logical paths are:
...
built-in rule that allows rearranging artifacts is `"install"`; a detailed
...
``` sh
$ jst build both
INFO: Requested target '""//:both' with config: {}
...
no longer be the case. Therefore, *jst-build* tries to be very
...
``` sh
$ jst analyse 'another greeter'
INFO: Requested target '""//:another greeter' with config: {}
