...
repository worked upon. Additional roots and file names (for targets,
rules, and expressions) can be specified. For keys not given, the same rules for
...
with the first entry specifying the naming scheme; the semantics
of the remaining entries depend on the scheme (see "Root Naming
Schemes" below).
...
The `"file"` scheme states that the repository (or respective
root) can be found in a directory in the local file system; the
only argument is the absolute path to that directory.

##### `"git tree"`

The `"git tree"` scheme states that the root is defined to be a
...
In the following, we assume it is located at `/etc/jst/repos.json`.
...
For example, the invocation `jst backend build -C /etc/jst/repos.conf
...
found at `/opt/foobar/repo` (using rules from `/etc/jst/rules`, and
taking `base` to refer to the repository at `/opt/barimpl`), and `bar`
...
In addition to the normal target references (a string for a target in
the same module, a module-target pair for a target in the same repository,
`./relpath:target` relative addressing, `jst.file(name)`
explicit file reference in the same module), references of the
form `repo//module:target` can be specified, where `repo` is a
...
upon (via `DEP_ARTIFACTS` and related functions). In the latter case,
...
as opaque values. More precisely,
 ...
 - during evaluation of the defining expression of the target's
...
target cannot use names unless it has those fields as a dependency anyway.
...
action. So for the consuming target it makes no difference whether its
dependency comes from the same repository or a different one.
