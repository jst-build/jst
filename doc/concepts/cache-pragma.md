...
This non-caching is achieved by re-requesting an action without cache
lookup if a failed action is reported from cache.
...
create reports on test suites). However, they break the connection
described above between caching and continuation of the build: we expect that
...
Another break with the functionality assumption of actions are "run"
...
technically pure but should still be used only in tests, e.g., a mock
...
The `"ACTION"` function in the defining expression of a rule has two
additional parameters (besides inputs, etc.), `"may_fail"` and
...
unchanged; in particular, they only describe the contents and not
whether they were obtained in a failed way.
...
or `install` command, if the requested artifacts contain a failed one, a
...
then only look at the failed tests in more detail. As we don't take
...
that installs a CAS entry, identified by the identifier shown in the
